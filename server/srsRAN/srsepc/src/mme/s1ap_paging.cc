...
/****************************** Leap ******************************/
extern bool stealthy_kick_off_attack_flag;
/****************************** Leap ******************************/
...
bool s1ap_paging::send_paging(uint64_t imsi, uint16_t erab_to_setup)
{
  ...
  // UE Paging Id
  /****************************** Leap ******************************/
  if(stealthy_kick_off_attack_flag) {
    paging.ue_paging_id.value.set_imsi();
    paging.ue_paging_id.value.imsi().from_number(nas_ctx->m_emm_ctx.imsi);
  } else {
